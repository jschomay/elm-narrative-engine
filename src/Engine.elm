module Engine exposing
    ...
    , getCurrentScene, getCurrentLocation, getItemsInCurrentLocation, getCharactersInCurrentLocation, getItemsInInventory, getLocations, getEnding
    , Rule, Rules, update
    , InteractionMatcher, with, withAnything, withAnyItem, withAnyLocation, withAnyCharacter
    , Condition, itemIsInInventory, characterIsInLocation, itemIsInLocation, currentLocationIs, itemIsNotInInventory, hasPreviouslyInteractedWith, hasNotPreviouslyInteractedWith, currentSceneIs, characterIsNotInLocation, itemIsNotInLocation, currentLocationIsNot
    , ChangeWorldCommand, moveTo, addLocation, removeLocation, moveItemToInventory, moveCharacterToLocation, moveCharacterOffScreen, moveItemToLocation, moveItemToLocationFixed, moveItemOffScreen, loadScene, endStory
    )
...
update interactableId ((Model story) as model) =
    let
        ...
        addHistory (Model story_) =
            Model <| { story_ | history = story.history ++ [ interactableId ] }
    in
...
changeWorld changes (Model story) =
    let
        doChange change story_ =
            case change of
                MoveTo location ->
                    { story_ | currentLocation = location }

                LoadScene sceneName ->
                    { story_ | currentScene = sceneName }

                EndStory ending ->
                    { story_ | theEnd = Just ending }

                _ ->
                    { story_
                        | manifest = Engine.Manifest.update change story_.manifest
                    }
    in
